 ...
 * @version 1.0
 */
public class ANL_Enge_DSSD_Simulation_2 extends Object {
	...
	public ANL_Enge_DSSD_Simulation_2() {
		DataSet[] stripCMtheta = new DataSet[16];

		double[] ExResidValues = { 8 };
		...
		double thetaYLSA = .06; //angle of array detectors
		try {
		...
		for (int rEx = 0; rEx < ExResidValues.length; rEx++) {
			...
			try {
				...
				int hits = 0;
				outEvn.write(
					"hit,projTheta,projPhi,CMtheta,CMphi,labTheta,labPhi,inc.,Edep,tof,strip\n");
				int _i = 0;
				...
				outEvn.flush();
				outEvn.close();
				outCounts.write("bin\tstrip\tcounts\tEmin\tEmax\n");
				//for (int i = 0; i < 5; i++) {
					for (int j = 0; j < 16; j++) {
						//int bin = i * 16 + j;
						int bin = j;
	...
	static public void main(String[] args) {
		new ANL_Enge_DSSD_Simulation_2();
