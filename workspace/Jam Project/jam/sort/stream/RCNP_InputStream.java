...
/**
 * This is edited form the YaleCAEN_InputStream file, to compensate for the 
 * usage of slot number 1 in the crate used at RCNP and that there are 31 
 * slots used.
 * 
 ...
 * @since       JDK1.1
 */
public class RCNP_InputStream extends L002HeaderReader implements L002Parameters {
    
    static final int NUM_V7X5_UNITS=1024;//32 slots in a VME crate - 1 controller slot
     ...
     * Default constructor.
     */
    public RCNP_InputStream(MessageHandler console) {
        super(console);
    }
     ...
     */
    public RCNP_InputStream(MessageHandler console, int eventSize) {
        super(console, eventSize);
    }
    
    private int [] tempParams=new int[64];
    private int [] tempData = new int[64];
    /**
    ...
    public synchronized EventInputStatus readEvent(int[] data) throws  EventException {
        ...
        int parameter=0;
        int endblock=0;
        final int [] tval =new int[64];	//temporary array for scaler values, up to a max of 64
        try {
             ...
             * when an event index gets added below */
                ...
                if (isHeader(header)) {
                    /* ADC's & TDC's in slots 1-31 */
                    int slot = (header >>> 27) & 0x1f;
                    int crate = (header >>> 27)  & 0x1f;
                    ...
                    while (keepGoing){                        
                        ...
                        if (isParameter(parameter)) {
                            ...
                            tempParams[paramIndex] = 32*crate+32*(slot-0)+channel;
