...
public class VME_Map_RCNP {
     ...
     *
     */
    public VME_Map_RCNP(SortRoutine sr) {
        sortRoutine = sr;
     ...
     */
    public int eventParameter(int slot, int crate,
    int channel, int threshold) throws SortException {
        VME_Channel_RCNP vmec = new VME_Channel_RCNP(this, slot, crate,
    ...
    VME_Channel_RCNP [] convertToArray(List vmeVec) {
        Object [] temp = vmeVec.toArray();
        VME_Channel_RCNP [] rval = new VME_Channel_RCNP[temp.length];
        for (int i=0; i< temp.length; i++) {
            rval[i] = (VME_Channel_RCNP)(temp[i]);
        }
        return rval;
    }

    public VME_Channel_RCNP [] getEventParameters(){
     ...
     */
    public void setV775Range(int crate, int range) throws SortException {
        //String hexBase = "0x"+Integer.toHexString(baseAddress);
        Integer ba=new Integer(crate);
        ...
        messages.append("A range of "+range+" ns requested for TDC with crate "+crate+
