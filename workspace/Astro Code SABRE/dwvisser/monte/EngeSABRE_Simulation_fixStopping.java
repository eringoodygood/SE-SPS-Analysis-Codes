 ...
 * 
 * Edited 6/9/15 for use with SABRE array by CMD.
 * Edited a variety of times by ECG for SABRE, most recently 11/9/22 for freezing on low energy states.
 * @author <a href="mailto:dale@visser.name">Dale W. Visser</a>
 * @version 1.0
 */
public class EngeSABRE_Simulation_fixStopping extends Object {
	static final double C_MM_PER_NSEC = 299.792458; //speed of light in the world's dumbest format.
	static final double DEAD_LAYER_THICKNESS = 0.05; //um. thick deadlayer are .5 um, the thin dead layer ones are .05 um. 
	...
	/** Creates new EngeSABRE_Simulation */
	public EngeSABRE_Simulation_fixStopping() {
		DataSet[] stripCMtheta = new DataSet[16];
						//7.651, 7.702,7.741, 7.789, 7.832, 7.893,7.913, 7.971, 8.037, 8.140, 8.184, 8.207, 8.226, 8.299, 8.324, 8.358, 8.38, 8.45, 8.525, 8.558, 8.587, 8.671
		double[] ExResidValues = {12.1746,12.2658,12.6637,12.7425, 13.2469, 14.434}; //value of state populated in excited nucleus BEFORE decay into sabre: MeV
		...
		System.err.println(
			"Total hits wanted: "
				+ hitsWanted
				+ " to get "
				+ (100 * fractionalErrorPerStrip) 
				+ "% error each strip");
		/**
		 * numEvents will be expanded as needed so that hitsWanted
		 * will be exceeded by approximately 20%
		 */
		//int numEvents = (int) Math.round(hitsWanted * 1.2);
		int numEvents = 10000000;
		//total events to simulate
		double z0 = 124.54; //distance in mm to vertex of array
		double thetaYLSA = 40; //angle of array detectors: back of array to detector face angle
		try {
			target = new Nucleus(13,27);
			beam = new Nucleus(2,3);
			projectile = new Nucleus(1, 2);
			decay = new Nucleus(2,4);
		} catch (NuclearException ne) {
			System.err.println(ne);
		}

		Ebeam = 24.0; //beam kinetic energy in MeV
		xtarg = 20.0; //thickness in ug/cm^2 (Al with C backing is only 150 Al, ignore backing)
		String starg = "C 1"; //material specification for target: THE STOICH
		theta = 3.0; //for spectrometer, in degrees
		double thetaAcceptance = 0.0302; 	// orig: 0.010 			//radians
		//choose "theta" of projectile <= +/- this
		double phiAcceptance = 0.040; //choose "phi" of projectile <= +/- this 			//orig: 0.040 is maximum opening... 
		double randomAcceptanceMax =
			Math.sqrt(
				Math.pow(thetaAcceptance, 2) + Math.pow(phiAcceptance, 2));
		ExUltimate = 0.0; //in MeV. Excited state in Final Nucleus. 
		...
		double hit_threshold = 0.0; //in MeV
		...
		for (int rEx = 0; rEx < ExResidValues.length; rEx++) {
			...
			String outFileRoot =
			      ...
			      "/Users/good073/Applications/SPS_code/sabre2024sims/"
					//+ "Ex_7.8_slit_val_"
			     // + thetaAcceptance;
			...
			try {
				...
				//MeV/c
				double EcmDecay =
					Math.sqrt(PcmDecay * PcmDecay + Mdecay * Mdecay);
				System.out.println(
					"Ex = "
						+ ExResid
				...
				do {
					_i++;
					//System.out.println(_i + "made it here!");
	...
	static public void main(String[] args) {
		new EngeSABRE_Simulation_fixStopping();
