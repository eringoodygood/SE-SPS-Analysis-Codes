 ...
 *
 * @author  Dale Visser
 * @version 1.0 (7 March 2001) //Edited by Erin for SABRE 2016
 */
public class MicronDetector extends Object implements WeightingFunction {
	...
	private static final double phiLimit = Math.toRadians(27.2); // (this is half of the detector wedge in degrees) the SABRE wedges subtend 54.4 deg. total of active area NOT INCLUDING INTERSTRIP REGIONS. 
	...
	public boolean isHit(Direction dir) {
		...
		hit = false;
		if (pr <= 135.1 && pr >= 32.6) {
			strip = (int) Math.floor((pr - 32.6) / 6.4); //this sets the strip geometry. a possible strip is (hit radius-innermost radius)/strip pitch
			double truncR = pr - 32.6 - 6.4 * strip; //calculates strip number? unsure. 
	...
	static public void main(String[] args) {
		...
		String fileRoot = "/Users/goode/Desktop/SPS Code/mcSims/";
		Direction d;
		boolean hit;
		double z0 = 124.54;
		double incline = 40;
		...
		int[][] distHists = new int[16][(110 - 75 + 1) * 5];	//just parameters for the file produced
		for (int strip = 0; strip < 16; strip++) {
			thetaHists[strip] = theta[strip].getHistogram(100, 170, 0.5); //likely more parameters
			incHists[strip] = inc[strip].getHistogram(0.8, 1.6, 0.02);
			distHists[strip] = dist[strip].getHistogram(75, 160, 0.2);
		}
		try {
			FileWriter incHist =
			...
			summary.write(
				"Array Geometric Efficiency = "							//part of output file
					+ 2.5 * p
					+ " +/- "
