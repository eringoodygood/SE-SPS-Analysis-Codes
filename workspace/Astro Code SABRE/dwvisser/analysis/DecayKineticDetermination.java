 ...
 * @version 1.0 (12 Jun 2001)
 */
public final class DecayKineticDetermination
	extends dwvisser.analysis.TextOutputter {
	/**
	 ...
	 */
	public DecayKineticDetermination(
		Nucleus target,
		Nucleus beam,
		Nucleus projectile,
		double beamEnergy,
		...
		try {
			...
			if (setTargetThickness(targetThickness, target))
				beamEnergy -= targetLoss.getEnergyLoss(beam, beamEnergy)
					/ 1000;
			reaction =
				new Reaction(
					target,
		...
		Nucleus ultimate =
			new Nucleus(
				reaction.getResidual().Z - decay.Z,
				reaction.getResidual().A - decay.A,
				lastExcitation);
		...
		Solid deadLayer = new Solid(getThicknessAl(milsAl), Absorber.CM, "Al");
	 ...
	 * @return  <code>true</code> if losses are to be accounted for
	 */
	...
	private double getLabAngle(double thetaDet, double phiDet) {
		double cosThetaRelative =
			Math.sin(thetaResid) * Math.sin(thetaDet) * Math.cos(phiDet)
	...
	private double getKinetic(double thetaLab) {
		...
		double Edecay =
			(Eresid * Msquare
				+ Presid
					* Math.cos(thetaLab)
					* Math.sqrt(Msquare * Msquare - Mdecay * Mdecay * 2 * denom))
				/ denom;
