 ...
 * @version 1.0
 */
public class ANL_Enge_DSSD_Simulation extends Object {
	...
	public ANL_Enge_DSSD_Simulation() {
		...
		double thetaYLSA = .10; //angle of array detectors
		try {
		...
		double thetaAcceptance = 0.0335;
		//choose "theta" of projectile <= +/- this
		double phiAcceptance = 0.2125; //choose "phi" of projectile <= +/- this
		...
		for (int rEx = 0; rEx < ExResidValues.length; rEx++) {
			...
			double Q = 2.08; //Q-value for reaction
			double excess = Mbeam + Mtarget + Q - Mresid - Mproj;
			try {
				if (excess < 0) {
				...
				do {
					...
					Direction residDirCM = 
						new Direction (
								Math.toRadians(
										Math.abs(reaction.getCMAngleResidual(0))),
								Math.PI + projDir.getPhi()); //this is the lab phi...for now i don't care
					...
					directionCM = residDirCM;
					double wtc = (Mtarget + Tbeam + Mbeam)
						/(1/Math.sqrt(1 - (Tbeam*Tbeam + 2*Tbeam*Mbeam)/Math.pow(Mtarget+Tbeam+Mbeam,2)));
					double PcmResid =
						Math.sqrt(
								wtc * wtc 
								- 2 * (Mproj*Mproj + Mresid*Mresid)
								+ Math.pow((Mresid*Mresid - Mproj*Mproj)/wtc, 2))
								/2;
					double EcmResid = 
						Math.sqrt(PcmResid*PcmResid + Mresid*Mresid);
					...
					System.arraycopy(directionCM.get3vector(PcmResid), 0, p_CM, 1, 3);
					p_lab = labBoost.transformVector(p_CM);
					...
					if (hit) {
						...
						double Tflight =
							Tinit
								- 0.001 * targetLossCalc.getEnergyLoss(
										residual,
										Tinit,
										Math.PI - directionLab.getTheta());
						/*tof =
				...
				outEvn.flush();
				outEvn.close();
				outCounts.write("bin\tring\tcounts\tEmin\tEmax\n");
				//for (int i = 0; i < 5; i++) {
					for (int j = 0; j < 16; j++) {
						//int bin = i * 16 + j;
						int bin = j;
