...
public class ANL_DSSD_Enge_sim_all_branch extends Object{
	 ...
	 * */
	public ANL_DSSD_Enge_sim_all_branch() {
		...
		double thetaAcceptance = 0.038;
		//choose "theta" of projectile <= +/- this
		double phiAcceptance = 0.038; //choose "phi" of projectile <= +/- this
		...
		for (int rEx = 0; rEx < ExResidValues.length; rEx++) {
			...
			do{
				...
				Direction residDir =
					new Direction(
						Math.toRadians(
							Math.abs(reaction.getLabAngleResidual(branch))),
						Math.PI + projDir.getPhi());
				
				Direction CMresidDir =new Direction(
						Math.toRadians(
								Math.abs(reaction.getCMAngleResidual(branch))),
							Math.PI + projDir.getPhi());
				
				Direction directionCM = CMresidDir;
				
				double ELabResid = reaction.getLabEnergyResidual(branch);
				
				p_lab[0] = ELabResid;
				System.arraycopy(
						residDir.get3vector(reaction.getLabMomentumResidual(branch)),
						0,
						p_lab,
						1,
						3);
			...
			outCounts.write("ring\tcounts\tEmin\tEmax\n");
				...
				outAngles.write("Theta\tCMcounts\tlabCounts\tProjLabCounts\n");
	...
	static public void main(String[] args) {
		new ANL_DSSD_Enge_sim_all_branch();
