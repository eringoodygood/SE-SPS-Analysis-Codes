...
public class MicronDetector extends Object implements WeightingFunction {
	...
	public boolean isHit(Direction dir) {
		...
		hit = false;
		if (pr <= 35 && pr >= 11.00) {
			ring = (int) Math.floor((pr - 11.0) / 1.5); //possible ring
			double truncR = pr - 11.0 - 1.5 * ring;
			/* excess radius from inner strip radius */
			if (ring < 16 && ring > -1) {
				double pzprime = pz - zDiff[ring];
	 ...
	 * 
	 * However, S2 is annular so all of phi is covered with single detector
	 * Therefore, phi discussion is moot. 
	 */
	static public Direction changePhi(Direction dir) {
		...
		double x = phi;
		return new Direction(dir.getTheta(), Math.toRadians(x));
	}
	...
	static public void main(String[] args) {
		...
		while (eventCount < numEventsToSimulate) {
			...
			if (hit) {
				...
				inc[ring].add(md.getIncidence());
				dist[ring].add(md.getDistance());
