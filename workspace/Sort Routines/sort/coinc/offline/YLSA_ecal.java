 ...
 */
public class YLSA_ecal extends SortRoutine {
    //VME properties
...
static final int [] TDC_BASE = {0x30000000,0x30010000};
    ...
    double [][] timePeak = new double[NUM_DETECTORS][STRIPS_PER_DETECTOR];
    ...
    public void initialize() throws Exception {
        ...
        try{
            LineNumberReader lr=new LineNumberReader(new FileReader("/data/jan01/calibration/selftime.out"));
            StreamTokenizer st = new StreamTokenizer(new BufferedReader(lr));
            st.eolIsSignificant(false); //treat end of line as white space
            do {
                st.nextToken();
                if (st.ttype != StreamTokenizer.TT_EOF){
                    int det=(int)Math.round(st.nval);
                    st.nextToken(); int str=(int)Math.round(st.nval);
                    st.nextToken(); double selfTimePeak=st.nval;
                    System.out.println("Det "+det+", Strip "+str+", self time peak = "+selfTimePeak);
                    timePeak[det][str]=selfTimePeak;
                }
            } while (st.ttype != StreamTokenizer.TT_EOF);
        } catch (IOException e) {
            System.err.println(e);
        ...
        for (int i=0; i<NUM_DETECTORS; i++) {
            for (int j=0; j<STRIPS_PER_DETECTOR; j++) {
                ...
                if (eTimes[i][j]==0) {
                    ...
                    eTimesGA[i][j]=Math.max(0,(int)Math.round(eTimes[i][j]+2500.0-timePeak[i][j]));
                }
