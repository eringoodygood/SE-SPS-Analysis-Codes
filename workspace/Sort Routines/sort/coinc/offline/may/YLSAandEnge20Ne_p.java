 ...
 * @since 24 April 2001
 */
public class YLSAandEnge20Ne_p extends SortRoutine {
    //VME properties
...
static final int [] TDC_BASE = {0x30000000,0x30010000,0x30020000};
    ...
    public void initialize() throws Exception {
        ...
        Nucleus F19=new Nucleus(9,19);
        M19=F19.getMass().value;
        ...
        hStateVsFP=new Histogram("StateVsFP",HIST_2D_INT,512,"Final State in 19F vs. FP Position",
        ...
        for (int i=0; i<NUM_DETECTORS; i++) {
            for (int j=0; j<STRIPS_PER_DETECTOR; j++) {
                ...
                if (time && energy) {
                    ...
                    deadLayerLoss.getThinEnergyLoss(proton,eDeposit,Math.acos(1/ac.getIncidence(j)));
        ...
        if (bGood && numInterHits == 0) {
            for (int hit = 0; hit<multiplicity; hit++){
                ...
                TWO_D_FACTOR,bin[hit])){
                    ...
                    Math.acos(1/ac.getIncidence(strip)));
                    ...
                    double Tproton = eFlight + 0.001*targetLoss.getThinEnergyLoss(
                    proton,eFlight,Math.toRadians(180.0)-thetaP);//eloss in keV
                    ...
                    //lab motion, mass of 19-F*
                    //cos(angle between 19-F and alpha)
                    double cosThetaRelative = Math.sin(zeta)*Math.sin(thetaP)*
                    Math.cos(phiP)+Math.cos(zeta)*Math.cos(thetaP);
                    //momentum^2 of 19-F
                    double P19sq = P4*P4 + Pproton*Pproton - 2.0*P4*Pproton*
                    cosThetaRelative;
                    double E19=E4-Eproton;//total energy of 19-F
                    double M19x = Math.sqrt(E19*E19-P19sq);
                    double Ex19 = M19x-M19;//state populated in 19-F
                    int plotBin = (int)Math.round(Ex19*10.0)+100;
