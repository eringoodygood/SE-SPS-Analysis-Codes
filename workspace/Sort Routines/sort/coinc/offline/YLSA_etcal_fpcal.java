 ...
 */
public class YLSA_etcal_fpcal extends SortRoutine {
    //VME properties
...
double [] testPhi = {-90.0,-18.0, 54.0, 126.0, 198.0};
    ...
    public void initialize() throws Exception {
        ...
        try{
            LineNumberReader lr=new LineNumberReader(new FileReader("/data/jan01/calibration/monte.out"));
            lr.readLine(); lr.readLine();
            lr.readLine(); lr.readLine();//skip first 4 lines
            StreamTokenizer st = new StreamTokenizer(new BufferedReader(lr));
            st.eolIsSignificant(false); //treat end of line as white space
            System.out.println("----\nAngles\n----");
            do {
                ...
                if (st.ttype != StreamTokenizer.TT_EOF){
                    ...
                    +theta[str]+", incidence = "+incidence[str]);
    ...
    public void sort(int [] dataEvent) throws Exception {
        ...
        ( gFrntRear.inGate(ecFPsn,ecRPsn) )){
            ...
            if (multiplicityOne &&gTime.inGate(eTimesGA[firstDetHit][firstStripHit]>>TWO_D_FACTOR,goodPIDstripBin)){
                ...
                /*if (firstDetHit==2){
                for (int i=0; i<testPhi.length;i++){
                phiAlpha = Math.toRadians(testPhi[i]);//lab phi of alpha
                cosThetaRelative = Math.sin(zeta)*Math.sin(thetaAlpha)*Math.cos(phiAlpha)+
                Math.cos(zeta)*Math.cos(thetaAlpha);//cos(angle between 16-O and alpha)
                P16sq = P4*P4+Palpha*Palpha-2.0*P4*Palpha*cosThetaRelative;//momentum^2 of 16-O
                E16=E4-Ealpha;//total energy of 16O
                M16x = Math.sqrt(E16*E16-P16sq);
                Ex16 = M16x-M16;//state populated in 16O
                //double Eplot = Ex-TalphaCM;//measures where in 16O we went
                plotBin = (int)Math.round(Ex16*10.0)+100;
                hTest[i].inc(plotBin);
                }
                }*/
            }
