 ...
 *
 */
public class Mg22offline extends SortRoutine {
    //VME properties
    ...
    Histogram hRYvsFY, hRYvsFYGPID,hFrntRearGY, hRYvsFYGFR;
    
    Histogram hSntrAnde, hSntrAndeG3, hFrntSntrGSA;
    Gate gSntrAnde;
    Histogram hFrntCthdM, hFrntSntrM, hSntrCthdM;//scint & cath transformed to average value
    Histogram hCthdAndeG3, hFrntAndeG3, hFrntSntrG5;
    Histogram hFrntSntrGSC, hFrntCthdGSC, hCthdAndeGSC;//gate by scintillator cathode
    Histogram hSntrCthdGFC, hFrntSntrGFC;//gate by Front wire Cathode
    Histogram hSntrCthdGFS, hFrntCthdGFS, hCthdAndeGFS;//gate by Front wire Scintillator
    Histogram hFrntGCSF, hRearGCSF, hFrntRearGPID, hFrntGAll, hRearGAll;//front and rear wire gate on all
    ...
    public void initialize() throws Exception {
        ...
        hRYvsFYGPID = new Histogram("RYvsFYGPID", HIST_2D_INT, TWO_D_CHANNELS,
        "Rear Y vs. Front Y, gated on PID minus trajectory", "Front Y", "Rear Y");
        hRYvsFYGFR = new Histogram("RYvsFYGFR", HIST_2D_INT, TWO_D_CHANNELS,
        "Rear Y vs. Front Y, Gated on Rear Psn vs. Front Psn", "Front Y", "Rear Y");
        hCthdAnde   =new Histogram("CathodeAnode  ", HIST_2D_INT,  TWO_D_CHANNELS, "Cathode vs Anode ","Cathode","Anode");
        hCthdAndeGFS=new Histogram("CathodeAnodeGFS", HIST_2D_INT,
        TWO_D_CHANNELS, "Cathode vs Anode - gated on FwSc","Cathode","Anode");
        hCthdAndeGSC=new Histogram("CathodeAnodeGSC", HIST_2D_INT,
        TWO_D_CHANNELS, "Cathode vs Anode - gated on ScCa","Cathode","Anode");
        ...
        hSntrCthdG3=new Histogram("ScintCathG3", HIST_2D_INT,  TWO_D_CHANNELS,
        "Cathode vs Scintillator - FwSc, FwCa, FwAn Gates","Scintillator","Cathode");
        ...
        hFrntSntrGSA=new Histogram("FrontScintGSA", HIST_2D_INT,  TWO_D_CHANNELS,
        "Scintillator vs Front Position - ScAn gate","Front Position", "Scintillator");
        hFrntSntrG5   =new Histogram("FrntSntrG5", HIST_2D_INT,  TWO_D_CHANNELS,
        "Scintillator vs Front Position, gated on other 5 PID + trajectory",
        "Front Position","Scintillator");
        hSntrAnde   =new Histogram("ScintAnode ", HIST_2D_INT,  TWO_D_CHANNELS,
        "Anode vs Scintillator","Scintillator","Anode");
        hSntrAndeG3   =new Histogram("ScintAnodeG3 ", HIST_2D_INT,  TWO_D_CHANNELS,
        "Anode vs Scintillator: gated on FrSc, FrCa, FrAn","Scintillator","Anode");
        ...
        //modified cath & scint
        hFrntCthdM =  new Histogram("FrontCathMod ", HIST_2D_INT,  TWO_D_CHANNELS,
        "Modified Cathode vs Front Position","Front Position","Cathode");
        hFrntSntrM   =new Histogram("FrontScintMod", HIST_2D_INT,  TWO_D_CHANNELS,
        "Modified Scintillator vs Front Position","Front Position","Scintillator");
        hSntrCthdM   =new Histogram("ScintCathMod", HIST_2D_INT,  TWO_D_CHANNELS,
        "Modified Cathode vs Modified Scintillator","Scintillator","Cathode");
        ...
        hFrntRearGPID=new Histogram("FRGatePID  ",HIST_2D_INT, TWO_D_CHANNELS,"Front vs. Rear - gated on PID minus trajectory");
        hFrntRearGY=new Histogram("FRGateY",HIST_2D_INT, TWO_D_CHANNELS,
        "Front vs. Rear - gated on Rear Y vs. Front Y");
        //gated on FwSc, FwCa, and ScCa
        hCthdAndeG3=new Histogram("CathodeAnodeG3", HIST_2D_INT,
        TWO_D_CHANNELS, "Cathode vs Anode - gated on FwSc,FwCa,FwAn","Cathode","Anode");
        hFrntAndeG3=new Histogram("FrontAnodeG3", HIST_2D_INT,
        TWO_D_CHANNELS, "Anode vs. Front - gated on FwSc,FwCa,ScCa","Front","Anode");
        //gated on 4 gates
        hFrntGAll   =new Histogram("FrontGAll    ", HIST_1D_INT, ADC_CHANNELS,
        "Front Position - ScCa,FwCa,FwSc,FwRw,CaAn gates");
        ...
        hFrntGES = new Histogram("FrontGES", HIST_1D_INT, ADC_CHANNELS,
        "Front Position - ScCa,FwCa,FwSc,FwRw,CaAn, EvsS, TimeBroad gates");
        //YLSA
        ...
        NUM_DETECTORS*STRIPS_PER_DETECTOR,"Channel vs. Energy", "Energy",
        "Channel");
        hEsilVsPspec = new Histogram("EsilVsPspec", HIST_2D_INT, TWO_D_CHANNELS,
        "YLSA Energy vs. Spectrometer Momentum -- gated on PID, TimeBroad, mult. 1",
        "Momentum [spectrometer]", "Energy [YLSA]");
        hEcmVsPspec = new Histogram("EcmVsPspec", HIST_2D_INT, TWO_D_CHANNELS,
        "YLSA CM Energy vs. Spectrometer Momentum -- gated on PID, TimeBroad, mult. 1",
        "Momentum [spectrometer]", "Energy [YLSA]");
        hTimeGAm1 = new Histogram("TimeGAm1",HIST_1D_INT, TWO_D_CHANNELS,
        "Time, Gain Adjusted, mult. 1");
        ...
        hFrntRearGPID.addGate(gFrntRear); hFrntRearGY.addGate(gFrntRear);
        gFRexclude = new Gate("FRexclude", hFrntPRearP);
        hFrntRearGPID.addGate(gFRexclude); hFrntRearGY.addGate(gFRexclude);
        gFrntAnde = new Gate("Fw-An", hFrntAnde);
        gFAalphas = new Gate("FAalphas", hFrntAnde);
        gFCalphas = new Gate("FCalphas", hFrntCthd);
        gFSalphas = new Gate("FSalphas", hFrntSntr);
        gSntrAnde = new Gate("An-Sc", hSntrAnde); hSntrAndeG3.addGate(gSntrAnde);
        gRYvsFY = new Gate("RYvsFY", hRYvsFY); hRYvsFYGPID.addGate(gRYvsFY);
        hRYvsFYGFR.addGate(gRYvsFY);
        ...
        hCthdAndeG3.addGate(gCthdAnde);
        hFrntAndeG3.addGate(gFrntAnde);
        hSntrCthdG3.addGate(gSntrCthd);
        hFrntSntrGSA.addGate(gFrntSntr);
        hFrntSntrG5.addGate(gFrntSntr);
    ...
    ClassNotFoundException{
        File data = new File("/data/jul01/calibrations/ArrayCalibration.obj");
    ...
    public void sort(int [] dataEvent) throws Exception {
        int cathN,cathTfm,scintN,scintTfm;
        ...
        int ecFY=eYF>>TWO_D_FACTOR;
        int ecRY=eYR>>TWO_D_FACTOR;
        ...
        double FRdiff=eRPsn-eFPsn-offset;
        int front=Math.max((int)(eFPsn-slopeF*FRdiff),0);
        if (eFPsn==0) front=0;
        ...
        if (ecSntr==0) {
            ...
            scintN=Math.max((int)(ecSntr-slopeS*FRdiff),0);
            scintTfm=Math.max(0,(int)(scintN-(cFront-139)*2.134864+16));
        }
        if (ecCthd==0) {
            cathN=0;
            cathTfm=0;
        } else {
            cathN=Math.max((int)(ecCthd-slopeC*FRdiff),0);
            cathTfm = Math.max(0, (int)(cathN-(cFront-139)*(-0.77116)-10));
        }
        int andeN=Math.max((int)(ecAnde-slopeA*FRdiff),0);
        if (ecAnde==0) andeN=0;
        ...
        hFrntY.inc(eYF);
        hRearY.inc(eYR);
        hYvsPsn.inc(cFront,ecFY);
        hRYvsFY.inc(ecFY,ecRY);
        ...
        boolean bSC, bCA, bFC, bFS, bFA, bSA, bPID, bState, bRoughPID;//all init as false (Java spec)
        boolean bRPvsFP = gFrntRear.inGate(ecFPsn,ecRPsn) && (!gFRexclude.inGate(ecFPsn,ecRPsn));
        boolean bRYvsFY = gRYvsFY.inGate(ecFY,ecRY);
        boolean bTrajectory =  bRPvsFP && bRYvsFY;
        bSC = gSntrCthd.inGate(scintN,cathN);
        bCA = gCthdAnde.inGate(cathN,andeN);
        bFC = gFrntCthd.inGate(cFront,cathN);
        bFS = gFrntSntr.inGate(cFront,scintN);
        bFA = gFrntAnde.inGate(cFront,andeN);
        bSA = gSntrAnde.inGate(scintN,andeN);
        bRoughPID = bFA&&bFS&&bFC;
        bPID = bRoughPID && bSC && bCA && bSA;
        bState = bPID && gPeak.inGate(front);
        if (bRPvsFP) hRYvsFYGFR.inc(ecFY,ecRY);
        if (bRYvsFY) hFrntRearGY.inc(ecFPsn,ecRPsn);
        if (bPID) {
            hFrntRearGPID.inc(ecFPsn,ecRPsn);
            hRYvsFYGPID.inc(ecFY,ecRY);
        }
        if (bRoughPID) {
            hFrntGCSF.inc(front);
            hRearGCSF.inc(eRPsn);
        ...
        if (bTrajectory){
            hFrntPH.inc(cFront,ecFHgh);
            hRearPH.inc(ecRPsn,ecRHgh);
            hCthdAnde.inc(cathN,andeN);
            hSntrCthd.inc(scintN,cathN);
            hSntrAnde.inc(scintN,andeN);
            hFrntCthd.inc(cFront,cathN);
            hFrntAnde.inc(cFront,andeN);
            hFrntSntr.inc(cFront,scintN);
            hFrntCthdM.inc(cFront,cathTfm);
            hFrntSntrM.inc(cFront,scintTfm);
            hSntrCthdM.inc(scintTfm,cathTfm);
            if (bFA && bFC && bSC && bCA && bSA) hFrntSntrG5.inc(cFront,scintN);
            if (bSC&&bFC&&bFS) {
                hFrntAndeG3.inc(cFront,andeN);
            }
            if (bRoughPID) {
                hCthdAndeG3.inc(cathN,andeN);
                hSntrCthdG3.inc(scintN,cathN);
                hSntrAndeG3.inc(scintN,andeN);
            }
            if (bSA) hFrntSntrGSA.inc(cFront,scintN);
            if (bSC) {// gate on Scintillator vs Cathode
                hFrntSntrGSC.inc(cFront,scintN);
                hFrntCthdGSC.inc(cFront,cathN);
                hCthdAndeGSC.inc(cathN,andeN);
            }
            if (bFC) {// gate on Front Wire Position vs Cathode
                hSntrCthdGFC.inc(scintN,cathN);
                hFrntSntrGFC.inc(cFront,scintN);
            }
            if (bFS){// gate on Front Wire Position vs Scintillator
                hSntrCthdGFS.inc(scintN,cathN);
                hFrntCthdGFS.inc(cFront,cathN);
                hCthdAndeGFS.inc(cathN,andeN);
            }
            if (bPID) {// gated on all 3 gate above
                writeEvent(dataEvent);
                hFrntGAll.inc(front);
                hRearGAll.inc(eRPsn);
                hcFrntGAll.inc(front>>COMPRESS_FACTOR);
                hcRearGAll.inc(eRPsn>>COMPRESS_FACTOR);
                hYvsPsnGPID.inc(cFront, eYF>>TWO_D_FACTOR);
            }
        }
        
        //Coincidence processing below commented out for faster execution
        ...
        for (int i=0; i<NUM_DETECTORS; i++) {
            for (int j=0; j<STRIPS_PER_DETECTOR; j++) {
                ...
                if (energy) hEnergyHits.inc(stripBin);
                ...
                if (time && energy) {
                    ...
                    boolean bTimeBroad = bTrajectory && bPID && gTimeBroad.inGate(ecTimeGA);
                    boolean bTimeState = bTrajectory && bState && gTimeState.inGate(ecTimeGA);
                    boolean bEvsS= gEvsS.inGate(eChannel,j);
                    boolean bTimeDecay = bTrajectory && bState && bEvsS &&
                    gTimeDecay.inGate(ecTimeGA);
                    if (bPID) hTimeGA.inc(ecTimeGA);
                    if (bTimeBroad) {
                        hEvsChBroad.inc(eChannel,stripBin);
                        hEvsStripBroad.inc(eChannel,j);
                        hFrntGTime.inc(front);
                        hcFrntGTime.inc(front>>COMPRESS_FACTOR);
                        if (bEvsS) hFrntGES.inc(front);
                    }
                    if (bState) {
        ...
        if (multiplicity==1){
            int i=detHit[0]; int j=stripHit[0];
            double eDeposit = ac.getEnergyDeposited(i,j,eEnergies[i][j]);//deposited in det [MeV]
            int ecTimeGA = eTimesGA[i][j] >> TWO_D_FACTOR;
            hTimeGAm1.inc(ecTimeGA);
            int eChannel = (int)Math.round(eDeposit*20);
            boolean bTimeBroad = bTrajectory && bPID && gTimeBroad.inGate(ecTimeGA);
            if (bPID && bTimeBroad) {
                hEsilVsPspec.inc(front>>COMPRESS_FACTOR,eChannel);
                hEcmVsPspec.inc(front>>COMPRESS_FACTOR,getEcmChannel(eEnergies[i][j],i,j,front));
            }
