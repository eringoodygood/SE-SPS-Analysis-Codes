 ...
 * @since 26 July 2001
 */
public class FvsCASpresort extends SortRoutine {
    //VME properties
...
static final int [] TDC_BASE = {0x30000000,0x30010000,0x30020000};
    ...
    Histogram hCthdAndeG3, hFrntAndeG3;
    ...
    public void initialize() throws Exception {
        ...
        TWO_D_CHANNELS, "Cathode vs Anode - gated on FwSc,FwCa,ScCa","Cathode","Anode");
    ...
    public void sort(int [] dataEvent) throws Exception {
        ...
        hFrntY.inc(eYF);
        hRearY.inc(eYR);
        hYvsPsn.inc(cFront,eYF>>TWO_D_FACTOR);
        ...
        /*boolean bSC = gSntrCthd.inGate(scintN,cathN);
        boolean bCA = gCthdAnde.inGate(cathN,andeN);
        boolean bFC = gFrntCthd.inGate(cFront,cathN);
        boolean bFS = gFrntSntr.inGate(cFront,scintN);
        boolean bPID = bSC && bFC && bFS && bCA;
        boolean bGood = bPID && gFrntRear.inGate(ecFPsn,ecRPsn);
        boolean bState = bGood && gPeak.inGate(front);*/
        
        boolean bFvsSACalphas = gFAalphas.inGate(cFront,andeN) || 
        gFCalphas.inGate(cFront,cathN) || gFSalphas.inGate(cFront,scintN);
        
        if (!bFvsSACalphas) writeEvent(dataEvent);
/*
        if (bSC&&bFC&&bFS) {
