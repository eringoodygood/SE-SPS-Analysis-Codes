 ...
 * @since 26 July 2001 
 */
public class Jac4adcs extends SortRoutine {
    //VME properties
    ...
    public void initialize() throws Exception {
        ...
        hEsilVsFP_noE = new Histogram("EsilVsFP_noE", HIST_2D_INT,TWO_D_CHANNELS,100,
        "E (YLSA) vs. Focal Plane Position, no \"energy condition\"","QBrho [channels]","E [MeV] * 10");
    ...
    /*private void readADCtresholds() throws IOException {
        String thresholdFile = "D:\\June02Analysis\\Lewising\\thresholds.txt";
    ...
    public void sort(int [] dataEvent) throws Exception {
        ...
        /* commenting out YLSA stuff for faster pre-sorting
        ...
        for (int i=0; i<NUM_DETECTORS; i++) {
            ...
            for (int j=0; j<STRIPS_PER_DETECTOR; j++) {
                ...
                //NO time calibration yet, so using raw time
                eTimesGA[i][j] = ac.getCalibratedTimeChannel(i,j,eTimes[i][j]);
                //eTimesGA[i][j] = eTimes[i][j];
                ...
                if (time) {
                    ...
                    if (gTvsStripGA_noE.inGate(ecTimeGA,stripBin)) {
                        ...
                        hEsilVsFP_noE.inc(ecFPsn, (int)Math.round(eDeposit*10));
                    }
                }
                if (time && energy) {
                    hHits.inc(stripBin);
        ...
        */ //commented out YLSA stuff for faster pre-sorting
