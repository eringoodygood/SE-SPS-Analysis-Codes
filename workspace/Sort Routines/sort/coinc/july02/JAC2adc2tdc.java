 ...
 * @since 26 July 2001
 */
public class JAC2adc2tdc extends SortRoutine {
    //VME properties
...
static final int [] TDC_BASE = {0x30000000,0x30010000};
    ...
    //LEDA Detectors
    static final int NUM_DETECTORS = 2;
    ...
    private int whichADCaddress(int detector){
        if (detector == 0 || detector == 1) {//Detectors 0 and 1 are all that are there
            return ADC_BASE[1];
        } else {
        	return -1;
        }
    ...
    private int whichADCchannel(int detector, int strip){
        if (detector == 0 ) {//top half
            return strip+16;
        } else {//detectors 1  bottom half
            return strip;
        }
    }
    /**
...
//            hBackPlane[i].inc(dataEvent[idBackPlane[i]]);
            for (int j=0; j<STRIPS_PER_DETECTOR; j++) {
                ...
                if (i>0 && i<3) hEnergies[i][j].inc(eEnergies[i][j]);
