 ...
 *
 * 17 August 2002--copied to sort.coinc.july02 for further modifications in analyzing
 * July data
 * 
 * @author Dale Visser
 * @since 26 July 2001
...
public class OfflineAnalysis extends SortRoutine {
	...
	Histogram hEsilGood,hEsilDecay,hEsilBkgd;
	Histogram hRYvsFY;
	...
	Histogram [] hQuadCoinc = new Histogram[4];
	Histogram [] hQuadBkgd = new Histogram[4];
	...
	public void initialize() throws Exception {
		...
		hEsilGood = new Histogram("EsilGood", HIST_1D_INT,1000,
		"Energy in YLSA w/ good PID in FP and EsilVsYLSA","100 ch/MeV","counts");
		hEsilDecay = new Histogram("EsilDecay", HIST_1D_INT,1000,
		"Energy in YLSA w/ good PID in FP and EsilVsYLSA & decay time gate","100 ch/MeV","counts");
		hEsilBkgd = new Histogram("EsilBkgd", HIST_1D_INT,1000,
		"Energy in YLSA w/ good PID in FP and EsilVsYLSA & bkgd time gate","100 ch/MeV","counts");
		hEvsStripState =
			new Histogram(
				"EvsSstate",
				HIST_2D_INT,
		...
		hTimeGA =
			new Histogram(
				...
				HIST_1D_INT,
				TWO_D_CHANNELS,
				"Time, Gain Adjusted, gated on PID & valid time");
		/*hTimeGAstate =
			new Histogram(
				"TimeGAstate",
				HIST_1D_INT,
		...
		hTimeGAdecay =
			new Histogram(
				...
				HIST_1D_INT,
				TWO_D_CHANNELS,
				"Time, Gain Adjusted, gated on PID, valid time & EsilVsFP");
		...
		for (int i=0; i<this.STRIPS_PER_DETECTOR; i++) {
			...
			//do groups of 4 strips
			if (i % 4 == 0) {
				int quad=i/4;
				hQuadCoinc[quad] =  new Histogram("QuadCoinc"+quad, HIST_1D_INT,
				this.ADC_CHANNELS, "Position New coincidence, strips "+(quad*4)+"-"+(quad*4+3));
				hQuadBkgd[quad] = new Histogram("QuadBkgd"+quad, HIST_1D_INT,
				this.ADC_CHANNELS, "Position New background, strips "+(quad*4)+"-"+(quad*4+3));
			}
		}
		...
		for (int i = 0; i < NUM_DETECTORS; i++) {
			for (int j = 0; j < STRIPS_PER_DETECTOR; j++) {
				...
				if (energy) {
					...
					int eDep100chPerMeV = (int)Math.floor(eDeposit * 100);//0=0-10 keV,1=10-20 keV, etc.
					...
					if (bEsilVsFP) hEsilGood.inc(eDep100chPerMeV);
					if (time) { //and energy since we are nexted
						...
						if (bGood) {
							...
							if (bEsilVsFP) {
								this.hTimeGAdecay.inc(ecTimeGA);
							}
						}
						if (bTimeBroad) {
