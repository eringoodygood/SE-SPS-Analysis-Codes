 ...
 * @since 26 July 2001
 */
public class FPplusYLSA_EdE extends SortRoutine {
    //VME properties
...
static final int [] TDC_BASE = {0x30000000,0x30010000,0x30020000};
    ...
    int idFrontBias, idRearBias, idBCIRange;
    Histogram hFrontBias, hRearBias, hBCIRange;
	...
	/**
	 * Histograms for containing E vs. deltaE for the silicon detector 
	 * Det 1/0 = Left delE/E
	 * Det 2/3 = Right delE/E
	 */
	Histogram [] leftEvsDE = new Histogram[this.STRIPS_PER_DETECTOR];
	Histogram [] rightEvsDE = new Histogram[this.STRIPS_PER_DETECTOR];
	Histogram leftEvsDE_all, rightEvsDE_all;
	final static int RIGHT_E=3;
	final static int LEFT_E=0;
	final static int RIGHT_DE=2;
	final static int LEFT_DE=1;
    ...
    int [] detEhit = new int[TOTAL_STRIPS];
    int [] stripEhit = new int[TOTAL_STRIPS];
    ...
    public void initialize() throws Exception {
        ...
        for (int i=0; i<this.STRIPS_PER_DETECTOR; i++){
        	this.leftEvsDE[i]=new Histogram("left EdE_"+i,HIST_2D_INT, TWO_D_CHANNELS,
        	"Left E vs. Delta-E, Strip "+i+" - 40 keV/ch",
        	 "E", "delta-E");
        	this.rightEvsDE[i]=new Histogram( "right EdE_"+i,HIST_2D_INT, TWO_D_CHANNELS, 
        	"Right E vs. Delta-E, Strip "+i+" - 40 keV/ch", "E", "delta-E");
        }
        this.leftEvsDE_all = new Histogram( "left EdE_all", HIST_2D_INT, TWO_D_CHANNELS, 
        "Left E vs. Delta-E, All, 40 keV/channel", "E", "delta-E");
        this.rightEvsDE_all = new Histogram( "right EdE_all", HIST_2D_INT, TWO_D_CHANNELS, 
        "Right E vs. Delta-E, All 40 keV/channel", "E", "delta-E");
        ...
        "Time vs. Strip, All Detectors", "Time", STRIPS_PER_DETECTOR+"*Det+Strip");
		...
		Gate gELeft = new Gate("ELeft", leftEvsDE_all);
		Gate gERight = new Gate("ERight", rightEvsDE_all);
        ...
        leftEvsDE_all.addGate(gELeft);
        rightEvsDE_all.addGate(gERight);
...
//            hBackPlane[i].inc(dataEvent[idBackPlane[i]]);
            for (int j=0; j<STRIPS_PER_DETECTOR; j++) {
                ...
                if (energy) {
                    detEhit[Emultiplicity]=i;
                    stripEhit[Emultiplicity]=j;
                	Emultiplicity++;
                ...
                if (time && energy) {
                    ...
                    if (bTimeBroad) {
                        ...
                        int _Senergy = (int)Math.round(ac.getEnergyDeposited(i,j,eEnergies[i][j])*10);                    	
                        if (multiplicity==1){
                        	hEsilVsFP_m1.inc(ecFPsn,_Senergy);
                        } else if (multiplicity==2){
                        	hEsilVsFP_m2.inc(ecFPsn,_Senergy);
                        } else if (multiplicity==3) {
                        	hEsilVsFP_m3.inc(ecFPsn,_Senergy);
                        }
        ...
        /**
         * Create E vs. delta-E plots for YLSA
         */
        for (int i=0; i<Emultiplicity; i++){
        	int det = detEhit[i];
        	int strip = stripEhit[i];
        	double E=0.0;
        	if (det==RIGHT_DE){
        		double delE = ac.getEnergyDeposited(det,strip,
        		eEnergies[det][strip]);//deposited in det [MeV]
        		if (eEnergies[RIGHT_E][strip]>0 && eEnergies[RIGHT_E][strip] <= LAST_ADC_BIN){
        			E = ac.getEnergyDeposited(RIGHT_E,strip,
        			eEnergies[RIGHT_E][strip]);//deposited in det [MeV]
        		}
                rightEvsDE[strip].inc((int)Math.round(E*25),(int)Math.round(delE*25));
                for (int rearStrip = strip-1; rearStrip <= strip+1; rearStrip++){
                	E=0.0;
                	if (rearStrip >=0 && rearStrip < this.STRIPS_PER_DETECTOR){
        				if (eEnergies[RIGHT_E][rearStrip]>0 && eEnergies[RIGHT_E][rearStrip] <= LAST_ADC_BIN){
                			E = ac.getEnergyDeposited(RIGHT_E,rearStrip,
        					eEnergies[RIGHT_E][rearStrip]);//deposited in det [MeV]
        				} 
					    rightEvsDE_all.inc((int)Math.round(E*25),(int)Math.round(delE*25));
                	}
                }
        	} else if (det==LEFT_DE){
        		double delE = ac.getEnergyDeposited(det,strip,
        		eEnergies[det][strip]);//deposited in det [MeV]
        		if (eEnergies[LEFT_E][strip]>0 && eEnergies[LEFT_E][strip] <= LAST_ADC_BIN){
        			E = ac.getEnergyDeposited(LEFT_E,strip,
        			eEnergies[LEFT_E][strip]);//deposited in det [MeV]
        		}
                leftEvsDE[strip].inc((int)Math.round(E*25),(int)Math.round(delE*25));
                for (int rearStrip = strip-1; rearStrip <= strip+1; rearStrip++){
                	E=0.0;
                	if (rearStrip >=0 && rearStrip < this.STRIPS_PER_DETECTOR){
                		if (eEnergies[LEFT_E][rearStrip]>0 && eEnergies[LEFT_E][rearStrip] <= LAST_ADC_BIN){
                			E = ac.getEnergyDeposited(LEFT_E,rearStrip,
        					eEnergies[LEFT_E][rearStrip]);//deposited in det [MeV]
                		}
                		leftEvsDE_all.inc((int)Math.round(E*25),(int)Math.round(delE*25));
                	}
                }
        	}
        }
	    ...
	    hFRhires.inc(eFPsn>>(TWO_D_HR_FACTOR),eRPsn>>(TWO_D_HR_FACTOR));
            if(bGood) {
                writeEvent(dataEvent);
                hFrntGAll.inc(eFPsn);
                hRearGAll.inc(eRPsn);
