 ...
 * @since 26 July 2001
 */
public class Mg24_offline extends SortRoutine {
    //VME properties
    ...
    DataParameter dp_XTheta, dp_XTheta2, dp_XTheta3, dp_XTheta5, dp_XPhi2,
    dp_XPhi4, dp_XThetaDelta, dp_XTheta2Delta, dp_XTheta3Delta;
    ...
    public void initialize() throws Exception {
        ...
        m6He = (new Nucleus(2,6)).getMass().value;
        rk=new ResidualKinematics(90,0,new Nucleus(8,16), new Nucleus(6,12),
        new Nucleus(2,4));
        ...
        dp_XTheta5=new DataParameter("(X|Theta^5) ");
        dp_XPhi2=new DataParameter("(X|Phi^2)    ");
        dp_XPhi4=new DataParameter("(X|Phi^4)   ");
        ...
        "Strip vs. Ecm(proton)","Energy[10 Ch/Mev]","Strip");
        hEcmVsFP = new Histogram("EcmVsFP", HIST_2D_INT,TWO_D_CHANNELS,100,
        "Ecm(proton) vs. Focal Plane Position","QBrho [channels]","Ecm [MeV]");
    ...
    public void sort(int [] dataEvent) throws Exception {
        ...
        double XTheta5=dp_XTheta5.getValue();     //use this to correct the dominant (x|theta^5) split-pole aberration
        double XPhi2 = dp_XPhi2.getValue();           // dominant phi aberration
        double XPhi4 = dp_XPhi4.getValue();
        ...
        int eCthdNew = (int)Math.round(eCthd-(theta-ADC_CHANNELS/2)/cathThetaSlope);
        int eAndeNew = (int)Math.round(eAnde-(theta-ADC_CHANNELS/2)/andeThetaSlope);
        int ePosnNew = (int)Math.round(eFPsn + delta*theta*(XThetaDelta + theta*(XTheta2Delta + 
        theta*XTheta3Delta)) + theta*(XTheta + theta *(XTheta2 + theta*XTheta3))+
        XTheta5*Math.pow(theta,5));
            ...
            for (int j=0; j<STRIPS_PER_DETECTOR; j++) {
                ...
                if (time && energy) {
                    ...
                    if (bTimeBroad) {
                        ...
                        try {
                            double eCMchannel= getEcmChannel(eEnergies[i][j],i,j,eFPsn);//MeV
                            hEcmVsStripBroad.inc((int)Math.round(eCMchannel*10),j);//1000 ch, 100 MeV F.S.
                            hEcmVsFP.inc(ecFPsn,(int)Math.round(eCMchannel));//100 ch, 100 MeV f.s.
    ...
    throws MathException {
    	//calibration points from 22Mg spectrum
