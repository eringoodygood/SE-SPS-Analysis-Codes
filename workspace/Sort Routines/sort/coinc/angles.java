 ...
 * @since 26 July 2001
 */
public class angles extends SortRoutine {
    //VME properties
...
static final int [] TDC_BASE = {0x30000000,0x30010000,0x30020000};
    ...
    Histogram hRearPH;
    Histogram hThetag, hPhig;
    Histogram hCthdAnde, hSntrCthd, hFrntCthd, hFrntAnde, hFrntSntr, hFrntPRearP;
    Histogram hFrntY, hRearY, hYvsPsn, hYvsPsnGPID, hFrntTheta, hFrntPhi;
    Histogram hThetaPhiG;
    ...
    Histogram hFrntThetaG, hFrntPhiG; // gated by all
    ...
    public void initialize() throws Exception {
        ...
        hThetag = new Histogram("Theta", HIST_1D_INT, ADC_CHANNELS, 
        "angle in x (dispersive) direction - gated");
        hPhig = new Histogram("Phi", HIST_1D_INT, ADC_CHANNELS, 
        "angle in y direction - gated");
        ...
        hFrntTheta = new Histogram("Theta vs Front", HIST_2D_INT, TWO_D_CHANNELS, 
        "Theta vs. Front Wire Position (X)", " Front Position", "Theta");
        hFrntPhi = new Histogram("Phi vs Front", HIST_2D_INT, TWO_D_CHANNELS, 
        "Phi vs. Front Wire Position (X)", " Front Position", "Phi");
        hFrntThetaG = new Histogram("Theta vs Front - Gated", HIST_2D_INT, 512, 
        "Theta vs. Front Wire Position (X)", " Front Position", "Theta");
        hFrntPhiG = new Histogram("Phi vs Front - Gated", HIST_2D_INT, 512, 
        "Phi vs. Front Wire Position (X)", " Front Position", "Phi");
        hThetaPhiG = new Histogram("Phi vs Theta - Gated", HIST_2D_INT, 512, 
    ...
    public void sort(int [] dataEvent) throws Exception {
        ...
        if (eSntr1 == 0) eSntr=eSntr2;
        if (eSntr2 == 0) eSntr=eSntr1;
	int eTheta = (int)Math.round((eFPsn - eRPsn)*4 + 2500);
	int ePhi = (int)Math.round((eYF - eYR)*3 + 2500);
	...
	hFrntTheta.inc(ecFPsn,eTheta>>TWO_D_FACTOR);
	hFrntPhi.inc(ecFPsn,ePhi>>TWO_D_FACTOR);   
            ...
            for (int j=0; j<STRIPS_PER_DETECTOR; j++) {
                ...
                if (time && energy) {
                    ...
                    if (bTimeBroad) {
                        ...
                        hFrntGTime.inc(eFPsn>>COMPRESS_FACTOR);
                    }
                    if (bState) {
        ...
        ************/
        for (int det=0; det<NUM_DETECTORS; det++){
            for (int strip=0; strip<STRIPS_PER_DETECTOR; strip++){
                if (eTimes[det][strip]>0) {
                    for (int Edet=0; Edet<NUM_DETECTORS; Edet++){
                        for (int Estrip=0; Estrip<NUM_DETECTORS; Estrip++){
                            if (eEnergies[Edet][Estrip] > 0  &&
                            eEnergies[Edet][Estrip] < LAST_ADC_BIN) {
                                hTvsEhits.inc(STRIPS_PER_DETECTOR*Edet+Estrip,
                                STRIPS_PER_DETECTOR*det+strip);
                            }
                        }
                    }
        ...
        if (bPID) {// gated on all 3 gate above
            ...
            if(bGood) {
                ...
                hFrntThetaG.inc(eFPsn>>3,eTheta>>3);
	        hFrntPhiG.inc(eFPsn>>3,ePhi>>3);
                hThetaPhiG.inc(eTheta>>3,ePhi>>3);
