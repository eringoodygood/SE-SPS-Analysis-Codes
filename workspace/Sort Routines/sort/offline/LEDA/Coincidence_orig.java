 ...
 */
public class Coincidence_orig extends SortRoutine {
    //VME properties
...
static final int [] TDC_BASE = {0x30000000,0x30010000};
    ...
    Histogram hFrntGT, hEvsP,hStripVsE,hStripVsP, hTvsE;
    ...
    public void initialize() throws Exception {
        ...
        for (int i=0; i<NUM_DETECTORS; i++) {
            for (int j=0; j<STRIPS_PER_DETECTOR; j++) {
                int Eparam=16 + 2*i*STRIPS_PER_DETECTOR + 2*j;
                int Tparam=Eparam+1;
                ...
                hStrips[i][j]=new Histogram("E_D"+i+"_S"+j, HIST_1D_INT, ENERGY_CHANNELS, "Detector "+i+", Strip "+j);
                hTimes[i][j]=new Histogram("T_D"+i+"_S"+j, HIST_1D_INT, TIME_CHANNELS, "Detector "+i+", Strip "+j+" time");
            }
            hTimeGPID[i]=new Histogram("TimeGPID_"+i,HIST_1D_INT, ADC_CHANNELS,
            "Detector "+i+", Strip "+SAMPLE_STRIP+" time gated on PID");
        }
    ...
    public void sort(int [] dataEvent) throws Exception {
        ...
        //for (int i=0; i<NUM_DETECTORS; i++) {
            //for (int j=0; j<STRIPS_PER_DETECTOR; j++) {
                int i=0;int j=4;
                int rawEnergy = dataEvent[idStrips[i][j]];
                eStrips[i][j] = (int)Math.round(rawEnergy * energyFactor[j]);
                eTimes[i][j] = (int)Math.round(dataEvent[idTimes[i][j]] * timeFactor[j]);
                if (rawEnergy > 0) multiplicity++;
            //}
        //}
        //boolean valid = (multiplicity == 1);
        ...
        //if (true) {
            //for (int i=0; i<NUM_DETECTORS; i++) {
                //for (int j=0; j<STRIPS_PER_DETECTOR; j++) {
                    ...
                    int ecEnergy=eStrips[i][j]>>TWO_D_ENERGY;
                    hEvsStrip.inc(ecEnergy,stripBin);
                    int ecTime=eTimes[i][j]>>TWO_D_TIME;
        ...
        ( gFrntRear.inGate(ecFPsn,ecRPsn) )){
            ...
            int numInGate=0;
            int whichStripBin=0;int energy=3;
            //for (int i=0; i<NUM_DETECTORS; i++) {
                //for (int j=0; j<STRIPS_PER_DETECTOR; j++) {
                    /*int*/ stripBin = i*STRIPS_PER_DETECTOR+j;
                    /*int*/ ecTime=eTimes[i][j]>>TWO_D_TIME;
                    if (gTime.inGate(ecTime,stripBin)) {
                        numInGate++;
                        whichStripBin=stripBin;
                        System.err.println("Det "+i+", Strip "+j+": "+eStrips[i][j]);
                        energy=eStrips[i][j]>>TWO_D_ENERGY;
                    }
                //}
            //}
            boolean inTimeGate=(numInGate==1);
            if (inTimeGate) {
                hFrntGT.inc(eFPsn);
                if (gPosn.inGate(eFPsn)){
                    hEvsP.inc(ecFPsn, energy);
                    hStripVsE.inc(energy, whichStripBin);
                    hStripVsP.inc(ecFPsn, whichStripBin);
                }
            }
            /*for (int i =0; i<NUM_DETECTORS;i++) {
                hTimeGPID[i].inc(eTimes[i][SAMPLE_STRIP]);
            }*/
        }
    }
