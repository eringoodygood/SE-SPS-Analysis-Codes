 ...
 */
public class SplitPoleOff extends SplitPoleOn {
...
/*	***check the addresses for these events***
	...
	idCthd=cnafCommands.event(1,3,0,0);	    //read first channel:Cathode signal
	idAnde=cnafCommands.event(1,3,1,0);	    //read Anode
	idScintR=cnafCommands.event(1,3,2,0);	    //read first channel:Cathode signal
	idScintL=cnafCommands.event(1,3,3,0);	    //read Anode
	idFrntPsn=cnafCommands.event(1,3,4,0);	    //read Front TAC
	idRearPsn=cnafCommands.event(1,3,5,0);	    //read Rear TAC
	idFrntHgh=cnafCommands.event(1,3,6,0);	    //read front wire pulse height
	idRearHgh=cnafCommands.event(1,3,7,0);	    //read rear wire pulse height
//	idGamma=cnafCommands.event(1,9,0,0);	    //read gamma ray energy (slot 9 channel 0)
	idMonitor=cnafCommands.event(1,9,1,0);      //read beam monitor	    (slot 9 channel 1)
//	idGamTac=cnafCommands.event(1,9,2,0);	    //read gamma ray Tac    (slot 9 channel 2)	
	
    	cnafCommands.event(1,3,12,11);		    //clear adc
	cnafCommands.event(1,9,12,11);		    //clear adc
	cnafCommands.event(1,20,0,10);		    //clear trigger module
...
*/
	...
	//gate on 3 gate 
	hFrntGCSF   =new Histogram("FrontGCSF    ", HIST_1D_INT, 2048, "Front Position - ScCa,FwCa,FwSc gates");   
	hRearGCSF   =new Histogram("RearGCSF    ", HIST_1D_INT, 2048, "Rear Position - ScCa,FwCa,FwSc gates");   
 ...
 //gate on front vs rear: write presort event file
