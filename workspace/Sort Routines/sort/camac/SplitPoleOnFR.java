 ...
 */
public class SplitPoleOnFR extends SortRoutine {
...
//  ***check the addresses for these events***
  ...
  cnafCommands.eventCommand(1,20,0,10);        //clear trigger module
      ...
      cnafCommands.scaler(1,16,4,0);  //scintillator
      cnafCommands.scaler(1,16,5,0);  //cathode
  ...
  hFrntRearGCSF=new Histogram("FRGateCSF  ",HIST_2D_INT, 512,"Front vs. Rear - ScCa, FwCa, FwSc gates");
  //gate on 3 gate
  ...
  if (( gSntrCthd.inGate(ecSntr,ecCthd) )&&
      ...
      ( gFrntSntr.inGate(ecFPsn,ecSntr) )){
        hFrntGCSF.inc(eFPsn);
        hRearGCSF.inc(eRPsn);
        hFrntRearGCSF.inc(eFPsn>>2,eRPsn>>2);
  }
  ...
  if (( gSntrCthd.inGate(ecSntr,ecCthd) )&&
    ...
    /*public double monitor(String name){
      ...
      if (name.equals("Live Time")){
        if (((double)(mEvntRt.getValue()))>0.0){
