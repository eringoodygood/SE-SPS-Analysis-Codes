 ...
 *
 * @author Ken Swartz, Alan Chen, and Dale Visser, Rachel Lewis
 * @version 7 Feb 2000 added clock to event stream
 */
public class SplitPoleOnClock extends SortRoutine {
    // ungated spectra
    ...
    Histogram hRearPH, hCthdAnde, hSntrCthd, hFrntCthd, hFrntAnde, hFrntSntr, hFrntPRearP;
    ...
    Histogram hFrntGCSF, hRearGCSF, hFrntGAll, hRearGAll;
    //front and rear wire gated on All compressed
    Histogram hcFrntGAll, hcRearGAll;
    //gates 1 d
    Gate gSilicon, gCthd, gPeak, gGood;
    //gates 2 d
    Gate gSntrCthd, gFrntSntr, gFrntCthd, gFrntRear;
    //scalers
    Scaler sBic, sClck, sEvntRaw, sEvntAccpt, sScint, sCath;
    //monitors
    Monitor mBeam, mClck, mEvntRt, mCthd, mGood, mLive;
    //id numbers for the signals;
    int idCthd, idAnde, idScintR, idScintL, idFrntPsn, idRearPsn;
    int idFrntHgh;    //front Wire Pulse Height
    int idRearHgh;    //Rear Wire Pulse Height
    int idFrntPH;    // position x height y
    int idRearPH, lastEvntAccpt;
    int idClock; // The clock scaler will be read into the event stream.
	...
	idClock=cnafCommands.eventRead(1,16,1,0);
	...
	//scaler reads
      	...
      	cnafCommands.scaler(1,16,4,0);	//read scintillator
      	cnafCommands.scaler(1,16,5,0);	// read cathode
