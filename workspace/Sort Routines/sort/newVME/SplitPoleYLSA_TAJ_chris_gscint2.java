 ...
 * @since 26 July 2001
 */
public class SplitPoleYLSA_TAJ_chris_gscint2 extends SortRoutine {
	//VME properties
...
//from TAJ
  ...
  private transient Histogram hCthd, hSntr1, hSntr2, hSntrSum,
	  hSntr1Sntr2, hFrntPsn, hRearPsn, hMonitor;
  /*
 ...
 // private transient Histogram /*hFWbias, hRWbias, */hBCIRange;
  ...
  private transient Histogram hFrntGCSF, hRearGCSF, hFrRightgCSF, hFrntGAll,
	...
	private transient Histogram hcFrntGTime, hFrntGTime;//front and rear wire gated on All compressed & time
  /*
  ...
  private transient DataParameter pXTheta, pThOffset, pCTheta;
   ...
   // Monitor [] mYLSArate = new Monitor[NUM_DETECTORS];
	...
	/*		hBCIRange = new Histogram("BCIRange", HIST_1D_INT, ADC_CHANNELS,
		...
		hEsilVsFP_gTime = new Histogram("EsilVsFP_gTime", HIST_2D_INT,TWO_D_CHANNELS,100,
		"E (YLSA) vs. Focal Plane Position gated on time peak","QBrho [channels]","E [MeV] * 10");
		hFPGAll_St = new Histogram("FrontGAll_St    ", HIST_1D_INT, ADC_CHANNELS,
		"Front Position - ScCa,FwCa,FwSc,FwRw gates and gated on EsilVsFP");
...
//taj
  final boolean XY_INGATE = gXY.inGate(FPOS_COMPR, FY_COMPR);
	  ...
	  boolean bState = bGood && gPeak.inGate(FPOS_COMPR);//from YLSA/OLD
	 
	 /* final boolean PROTONS = gSntrCthd_p.inGate(SCINT_COMPR, ecCthd)
		   && gFrntSntr_p.inGate(FPOS_COMPR, SCINT_COMPR)
		   && gFrntCthd_p.inGate(FPOS_COMPR, ecCthd) && ACCEPT;
	  final boolean DEUTERONS = gSntrCthd_d.inGate(SCINT_COMPR, ecCthd) &&
		  gFrntSntr_d.inGate(FPOS_COMPR, SCINT_COMPR)
		   && gFrntCthd_d.inGate(FPOS_COMPR, ecCthd)
		   && ACCEPT;
	  final boolean TRITONS = gSntrCthd_t.inGate(SCINT_COMPR, ecCthd) &&
		  gFrntSntr_t.inGate(FPOS_COMPR, SCINT_COMPR)
		   && gFrntCthd_t.inGate(FPOS_COMPR, ecCthd)
		   && ACCEPT;
	  final boolean ALPHAS = gSntrCthd_a.inGate(SCINT_COMPR, ecCthd) &&
		  gFrntSntr_a.inGate(FPOS_COMPR, SCINT_COMPR)
		   && gFrntCthd_a.inGate(FPOS_COMPR, ecCthd)
		   && ACCEPT;
*/
...
//			  hBackPlane[i].inc(dataEvent[idBackPlane[i]]);
			for (int j=0; j<STRIPS_PER_DETECTOR; j++) {
				...
				if (time && energy) {
					...
					if (bPID) {
						...
						hEsilVsFP.inc(FPOS_COMPR,_Senergy);  //CHANGED!!
					...
					if (bTimeBroad) {
						...
						int _Senergy = (int)Math.round(ac.getEnergyDeposited(i,j,eEnergies[i][j])*10);  
						hEsilVsFP_gTime.inc(FPOS_COMPR,_Senergy);    
						boolean bEState = gEsilVsFP.inGate(FPOS_COMPR,_Senergy);
							if(bEState){
								hFPGAll_St.inc(FPOS);
							}              	
						if (multiplicity==1){
	...
	/*		if (PROTONS) {
			  // gated on proton gates plus acceptance gates
			  hFrPosProton.inc(FPOS);
			}
			if (DEUTERONS) {
			  // gated on deuteron gates plus acceptance gates
			  hFrPosDeuts.inc(FPOS);
			  if (gPeak.inGate(FPOS)) {
				hCathTheta.inc(eCthd / 4, THETA_CHANNEL);
			  }
			}
			if (TRITONS) {
			  // gated on proton gates plus acceptance gates
			  hFrPosTriton.inc(FPOS);
			}
			if (ALPHAS) {
			  // gated on proton gates plus acceptance gates
			  hFrntGAll_a.inc(FPOS);
			}
		  */
