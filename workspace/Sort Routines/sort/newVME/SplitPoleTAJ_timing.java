 ...
 * @since 26 July 2001
 */
public class SplitPoleTAJ_timing extends SortRoutine {
    //VME properties
...
static final int [] TDC_BASE = {0x30000000,0x30020000};
    ...
    // ungated spectra
    Histogram hCthd,hMonitor, hMonitor_t1, hMonitor_t2, hMonitor_t3, hMonitor_t4, hMonitor_t5, hMonitor_t6, hMonitor_t7, hMonitor_t8, hMonitor_t9, hMonitor_t10,hSntr1,hSntr2,hSntrSum, hFrntPsn, hRearPsn;
    ...
    int idCthd, idMonitor, idScintR, idScintL, idFrntPsn, idRearPsn, idFrntHgh,
...
// TDC based position parameters
	...
	idRRTDC = vmeMap.eventParameter(7,TDC_BASE[1], 1, TIME_THRESHOLDS);
        ...
        "Cathode Raw ");
        hMonitor      =new Histogram("Monitor       ", HIST_1D_INT, ADC_CHANNELS,
        "Monitor");
		hMonitor_t1      =new Histogram("Monitor time 1      ", HIST_1D_INT, ADC_CHANNELS,
		 "Monitor");
		hMonitor_t2      =new Histogram("Monitor time 2      ", HIST_1D_INT, ADC_CHANNELS,
		 "Monitor");
		hMonitor_t3      =new Histogram("Monitor time 3      ", HIST_1D_INT, ADC_CHANNELS,
		 "Monitor");
		hMonitor_t4      =new Histogram("Monitor time 4      ", HIST_1D_INT, ADC_CHANNELS,
		 "Monitor");
		hMonitor_t5      =new Histogram("Monitor time 5      ", HIST_1D_INT, ADC_CHANNELS,
		 "Monitor");
		hMonitor_t6      =new Histogram("Monitor time 6      ", HIST_1D_INT, ADC_CHANNELS,
		 "Monitor");
		hMonitor_t7      =new Histogram("Monitor time 7      ", HIST_1D_INT, ADC_CHANNELS,
		 "Monitor");
		hMonitor_t8      =new Histogram("Monitor time 8      ", HIST_1D_INT, ADC_CHANNELS,
		 "Monitor");
		hMonitor_t9      =new Histogram("Monitor time 9       ", HIST_1D_INT, ADC_CHANNELS,
		 "Monitor");
		hMonitor_t10      =new Histogram("Monitor time 10      ", HIST_1D_INT, ADC_CHANNELS,
		 "Monitor");
...
//		Parameters
    ...
    public void sort(int [] dataEvent) throws Exception {
        ...
        //added 7/2/07 to sort time segments of runs
        
		
        
        int eCthd   =dataEvent[idCthd];
        int eMonitor   =dataEvent[idMonitor];
       ...
       //int eCthdAnde = (eCthd + eAnde)/2;
		...
		int time_total=44480;
		int t1=time_total*(1/10);
		int t2=time_total*(2/10);
		int t3=time_total*(3/10);
		int t4=time_total*(4/10);
		int t5=time_total*(5/10);
		int t6=time_total*(6/10);
		int t7=time_total*(7/10);
		int t8=time_total*(8/10);
		int t9=time_total*(9/10);
		int time = sBic.getValue();
				if(time>=0 && time<4448){
        hMonitor_t1.inc(eMonitor);
				}
		if(time>=4448 && time<8896){
				hMonitor_t2.inc(eMonitor);
						}
		if(time>=8896 && time<13344){
						hMonitor_t3.inc(eMonitor);
								}
		if(time>=13344 && time<17792){
						hMonitor_t4.inc(eMonitor);
								}
		if(time>=17792 && time<22240){
						hMonitor_t5.inc(eMonitor);
								}
		if(time>=22240 && time<26688){
						hMonitor_t6.inc(eMonitor);
								}
		if(time>=26688 && time<31136){
						hMonitor_t7.inc(eMonitor);
								}
		if(time>=31136 && time<35584){
						hMonitor_t8.inc(eMonitor);
								}
		if(time>=35584 && time<40032){
						hMonitor_t9.inc(eMonitor);
								}
		if(time>=40032 && time<=time_total){
						hMonitor_t10.inc(eMonitor);
								}
        hSntr1.inc(eSntr1);
        hSntr2.inc(eSntr2);
        hSntrSum.inc(eSntr);
