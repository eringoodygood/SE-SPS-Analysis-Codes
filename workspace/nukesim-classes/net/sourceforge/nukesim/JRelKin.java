...
import net.sourceforge.nukesim.nuclear.swing.MassTableChooserMenuItems;
import net.sourceforge.nukesim.nuclear.swing.ValuesChooser;
import net.sourceforge.nukesim.nuclear.swing.ValuesListener;
import net.sourceforge.nukesim.nuclear.table.KinematicsOutputTable;
import net.sourceforge.nukesim.nuclear.table.KinematicsOutputTableModel;
import net.sourceforge.nukesim.nuclear.table.ReactionTable;
import net.sourceforge.nukesim.nuclear.table.ReactionTableModel;
 ...
 * @version 1.0
 */
public final class JRelKin extends JFrame implements ValuesListener, ActionListener, NukeUnits {
	...
	private static final String VERSION=(new StringBuffer(
	VERSION_MAJOR).append('.').append(VERSION_MINOR).append(
	...
	private final ValuesChooser la3 =
		new ValuesChooser(this, "Lab \u03b8(Projectile)", deg, INITIAL_LAB_ANGLES);
	private final JTextField tt = new JTextField("0.0");

	private final Container pane;
	private PageFormat mPageFormat;
	private final KinematicsOutputTable kot;	
	...
	public JRelKin() {
		super("JRelKin");
		new SplashWindow(this, 10000, "net/sourceforge/nukesim/jrelkin96.png", getTitle(), "v 1.2");
		final ClassLoader cl = getClass().getClassLoader();
		final Image icon =
			(new ImageIcon(cl.getResource("net/sourceforge/nukesim/jrelkin96.png")).getImage());
		...
		try {
			kotm =
				new KinematicsOutputTableModel(
					rt,
					energiesOf(be.getValues()),
					energiesOf(ex4.getValues()),
					anglesOf(la3.getValues()));
		...
		kot = new KinematicsOutputTable(kotm);
		setupMenu();
	...
	private final void setupMenu() {
		...
		final MassTableChooserMenuItems mtcmi = new MassTableChooserMenuItems(pref,rt);
		mtcmi.addListener(kot); 
		mbar.add(pref);
	...
	public boolean receiveValues(ValuesChooser vc, double [] values) {
		boolean rval = true;
		final Unit units=vc.getUnits();
		final int len=values.length;
		final Quantity [] qValues=new Quantity[len];
		for (int i=0; i<len; i++){
			qValues[i]=Quantity.valueOf(values[i],units);
		}
		try {
			if (vc == be) {
				kotm.setBeamEnergies(energiesOf(qValues));
			} else if (vc == ex4) {
				kotm.setResidualExcitations(energiesOf(qValues));
			} else if (vc == la3) {
				kotm.setLabAngles(anglesOf(qValues));
			}
		} catch (KinematicsException ke) {
			JOptionPane.showMessageDialog(
				this,
	...
	private Energy [] energiesOf(Quantity [] values){
		final int len=values.length;
		final Energy [] rval=new Energy[len];
		for (int i=0; i<len; i++){
			rval[i]=Energy.energyOf(values[i]);
		}
		return rval;
	}
	
	private Angle [] anglesOf(Quantity [] values){
		final int len=values.length;
		final Angle [] rval=new Angle[len];
		for (int i=0; i<len; i++){
			rval[i]=Angle.angleOf(values[i]);
		}
		return rval;
	}
	...
	public void actionPerformed(final java.awt.event.ActionEvent ae) {
		...
		if (command.equals("i_ok")) {
			...
			try {
				kotm.setTargetThickness(Quantity.valueOf(thickness,µg_per_cmsq));
			} catch (KinematicsException ke) {
				JOptionPane.showMessageDialog(
					this,
					ke.getMessage(),
