...
public class TargetDefinitionTableModel extends DefaultTableModel {
    ...
    private void setThicknesses(){
        for (int i=0; i < _target.getNumberOfLayers(); i++){
            setValueAt(new Double(_target.getLayer(i).getThickness().doubleValue(NukeUnits.µg_per_cmsq)),i,2);
        }
    }
    ...
    Target makeTarget(String name) throws NuclearException {
        ...
        for (int row=0; row < getRowCount(); row++){
            ...
            final Solid layer=new Solid((String)spec,Quantity.valueOf(thickness,NukeUnits.µg_per_cmsq));
            rval.addLayer(layer);
        }
        return rval;
    }
