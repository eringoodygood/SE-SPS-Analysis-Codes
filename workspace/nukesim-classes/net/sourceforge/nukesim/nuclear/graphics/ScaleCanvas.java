...
/**
 * Designed to be displayed under SpectrumCanvas and give the
 * scale in length units.
 *
 * @author  Dale Visser
 */
public class ScaleCanvas extends JPanel implements NukeUnits{

	private final static int HEIGHT=25;
    private final RadiusRange range;
    ...
    public void paintComponent(Graphics graphics) {
        ...
        final double min=range.getRhoMin().doubleValue(cm);
        final double max=range.getRhoMax().doubleValue(cm);
        final double length=max-min;
        g2.setPaint(Color.BLACK);
        double lastStringX=d.width;
        final int [] radii = ticksLinear();
        for (int i=radii.length-1; i>=0;i--){
            if (radii[i] > min && radii[i] < max){
            	final String label=String.valueOf(radii[i]);
                final double xfrac = (radii[i]-min)/length;
                final double xcoord = xfrac*d.width;
				final Rectangle2D stringBounds=g2.getFont().getStringBounds(label,frc);
                final double width=stringBounds.getWidth();
                final double stringX = xcoord-width/2;
                final double diff = lastStringX-xcoord;
                final double lineBottom;
                if (diff >= width) {
                    g2.drawString(label,(float)stringX,(float)(d.height-
	...
	private int[] ticksLinear() {
		double lowerLimit=range.getRhoMin().doubleValue(cm);
		double upperLimit=range.getRhoMax().doubleValue(cm);
