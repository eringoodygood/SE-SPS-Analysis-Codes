...
public class Matrix {
	...
	// constructor method for matrix add, product and so on
		if (((code == '+')||(code == '-'))&&(m1.rows == m2.rows)&&(m1.columns==m2.columns)) {
			// add
			rows = m1.rows;
			columns = m1.columns;
			element = new double[rows][columns];
			...
			for (i=0; i<rows; i++) {
			  for (j=0; j<columns; j++) {
			    switch (code) {
			    	case '+': element[i][j] = m1.element[i][j] + m2.element[i][j]; break;
			    	case '-': element[i][j] = m1.element[i][j] - m2.element[i][j]; break;
			    }
			  }
			}
			...
			*/
			...
			element[rows-1][0]=m2.element[rows-1][0]/m1.element[rows-1][rows-1];
	...
	Vector col = new Vector();	// and then transferred into the array element[][].
	...
	*/
		...
		Matrix P = new Matrix(rows, columns, 'I');	// P will track the permutations
		Matrix L = new Matrix(rows, columns, 'I');	// the lower triangle
		Matrix U = this;	// this matrix to be transformed to upper triangular
		Matrix G = new Matrix(rows, columns, 'I');	// temporary Gauss transform matrix
		int i, j, k;
	...
	*/
	...
	public String toStringUL(int d) {
		// this method returns a string representation of the matrix
		// d displayed fractional digits
		if (this.rows!=this.columns) return ("Error: toStringUR() not square!");
		NumberFormat nf = NumberFormat.getInstance();
		nf.setMaximumFractionDigits(d);
		nf.setMinimumFractionDigits(d);
		String outPut = new String();
