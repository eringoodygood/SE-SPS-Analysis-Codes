/*
 * YLSAcalibration.java
 *
 * Created on February 14, 2001, 11:17 AM
 */
...
public class BatchCalibration extends Object {
     ...
     * @param acFile database of calibration info
     */
            ...
            for (int d=0; d<ndet; d++){
                for (int s=0; s<nstr; s++){
                    if (is.dataExists(d,s)){
                        ...
                        Multiplet [] fits = doFits(spectrum, results, width);//temporary 2.8 and 3 are to copy above
                        Multiplet mfinal = Multiplet.combineMultiplets(fits);
                        int numRemaining = mfinal.removeAreaLessThan(300.0);//minimum area requirement of top 3 peaks
            ...
            for (int d=0; d<ndet; d++){
                for (int s=0; s<nstr; s++){
                    if (is.dataExists(d,s)){
                        ...
                        if (numRemaining < 3) {
                            //do nothing not enough usable data here
                        } else {
                            for (int i=0; i<T.length; i++){
            ...
            for (int d=0; d<ndet; d++){
                for (int s=0; s<nstr; s++){
                    if (is.dataExists(d,s)){
                        ...
                        int numRemaining = mfinal.removeAreaLessThan(3000.0);//minimum area requirement of top 3 peaks
     ...
     */
    private void printResults(int [] spectrum, String name,
    Multiplet [] results){
