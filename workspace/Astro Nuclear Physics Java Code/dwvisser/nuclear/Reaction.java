...
public class Reaction {
    ...
    double w2l,p2l,g2l,b2l;//total energy, momentum, gamma, beta of 2 in lab
    ...
    throws KinematicsException {
        ...
        /*System.err.println("new Reaction("+target+","+beam+","+projectile+","+
        Tbeam+" MeV,"+thetaLab+" deg,"+residualExcitation+" MeV)");*/
        this.target=target;
        this.beam=beam;
     ...
     */
    public Reaction(Nucleus target, Nucleus beam, Nucleus projectile) throws
    KinematicsException {
     ...
     * @throws KinematicsException if a calculation problem occurs
     */
    public Reaction(Reaction r, double eBeam, double thetaLab, double residExcite)
    throws KinematicsException {
        this(r.getTarget(),r.getBeam(),r.getProjectile(),eBeam,
        thetaLab,new UncertainNumber(residExcite));
    }
    ...
    public UncertainNumber getEx4(UncertainNumber labMomentum3){
        ...
        double p4lSq,p3l,w4l,w3l; //local p3l,w3l,w4l are double, not double []
        ...
        bestEstimate = Math.sqrt(w4l*w4l-p4lSq)-groundState.getMass().value;
        ...
        w4l = wtl-w3l;
        groundState=new Nucleus(residual.Z,residual.A); //implicitly g.s.
        devUpper = Math.sqrt(w4l*w4l-p4lSq)-groundState.getMass().value;
        ...
        w4l = wtl-w3l;
        groundState=new Nucleus(residual.Z,residual.A); //implicitly g.s.
        devLower = Math.sqrt(w4l*w4l-p4lSq)-groundState.getMass().value;
        devLower = Math.abs(devLower-bestEstimate);
        //System.out.println("Deviation, lo p = "+devLower);
    ...
    public void printStatus(){
        ...
        for (int i=0;i<t3c.length;i++){
            ...
            System.out.println("Momentum of 3, lab["+i+"] = "+p3l[i]+" MeV/c");
        }
    }
     ...
     * The workhorse.
     */
    private void setKinematicQuantities() {
        ...
        } else if (calculateOption == Reaction.UNCERTAIN_PROJECTILE_MASS_OPTION){
            m3 += projectile.getMass().error;
        } else if (calculateOption == Reaction.UNCERTAIN_RESIDUAL_MASS_OPTION){
        ...
        if (wtc > (m3+m4)){//Total energy greater than rest mass of products
            calculated=true;
            ...
            xx=(m1-m4)*(m1+m4)+(m2-m3)*(2.0*m1+m2-m3)+2.0*eb*(m1-m3);
            aa=ct3l*ct3l+g*g*st3l*st3l;
            bb=g*g*st3l*st3l*2.0*k3c;
            cc=4.0*ct3l*ct3l*(ct3l*ct3l+g*g*st3l*st3l*xx*(g3c/g+1.0)/((g3c*g3c-1.0)*
            (2.0*m3*wtl)));
    ...
    private void setQ(){
        Q=target.getMass().plus(beam.getMass()).minus(projectile.getMass()).minus(
        residual.getMass());
    }
    ...
    private void COMtoLab(){
        ...
        for (int i=0;i<t3c.length;i++){
            ...
            if (t4c[i] * t4l[i] < 0.0) t4l[i]=t4l[i]-Math.PI;
            w3l[i]=w3c*g*(1.0+b*b3c*ct3c);
            e3l[i]=w3l[i]-m3;
            w4l[i]=w4c*g*(1.0+b*b4c*Math.cos(t4c[i]));
            e4l[i]=w4l[i]-m4;
            if (1.0+k3c*ct3c > 0.0) {
                jac3[i] = Math.pow(st3c*st3c+g*g*Math.pow(k3c+ct3c,2.0),1.5);
    ...
    public int getAngleDegeneracy(){
        if (overMax || !calculated) return 0;
        return t3c.length;
    }
    ...
    static public UncertainNumber getQBrho(Nucleus nuke, UncertainNumber KE, boolean varyMass){
        Vector deviations = new Vector(2);
        double value = Reaction.getQBrho(nuke, KE.value);
        deviations.add(new Double(Reaction.getQBrho(nuke,KE.value+KE.error)-value));
        if (varyMass) {
    ...
    public String toString(){
        return target.toString()+"("+beam.toString()+","+projectile.toString()+
        ")"+residual.toString();
    }
...
//        new Nucleus();
        ...
        double ex=0;
        try {
            Reaction r = new Reaction(target,beam,projectile,energy,theta,ex);
            System.out.println("Exact QBr : "+r.getQBrho(0,Reaction.EXACT_OPTION));
            ...
            System.out.println("Variable projectile mass = "+projectile.getMass()+" : "+
            r.getQBrho(0,Reaction.UNCERTAIN_PROJECTILE_MASS_OPTION));
            System.out.println("Variable residual mass = "+r.getResidual().getMass()+" : "+
            r.getQBrho(0,Reaction.UNCERTAIN_RESIDUAL_MASS_OPTION));
            System.out.println("Vary all masses: "+r.getQBrho(0,Reaction.UNCERTAIN_BEAM_MASS_OPTION |
            Reaction.UNCERTAIN_PROJECTILE_MASS_OPTION | Reaction.UNCERTAIN_RESIDUAL_MASS_OPTION |
