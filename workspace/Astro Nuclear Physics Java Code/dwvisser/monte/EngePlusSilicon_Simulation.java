...
public class EngePlusSilicon_Simulation extends Object {
    ...
    public EngePlusSilicon_Simulation() {
        /*****
         * Put nuclear/stopping intialization code here.
         *****/
//        new EnergyLoss();
        Direction generator = new Direction(0,0);
        
        double [] ExResidValues = {5.673};
        /*** Initial values put here ***/
        int hitsWanted = 100*16; //hits wanted ~ (fractional error wanted per strip)^-2
        /**
        ...
        ExUltimate = 0;//in MeV
        int angularMomentum = 0;//assumed orbital angular momentum for the decay
        double AlThickness = 0;//thickness of degrading foil in front of detector in mils
        //at this time, simply added to dead layer thickness, assuming we would
        //put the foils parallel to the detectors
        double hit_threshold = 0.0; //energy deposited in MeV to be considered a hit
        ...
        for (int rEx = 0; rEx<ExResidValues.length; rEx++){
            ...
            ultimate = new Nucleus(residual.Z - decay.Z, residual.A - decay.A,
            ExUltimate);
            String whetherForward="_back";
            if (arrayForward) whetherForward="_front";
            String outFileRoot = "d:/simulations/"+
            (int)Math.round(ExResid*1000)+"_"+residual+"_"+
            (int)Math.round(ExUltimate*1000)+"_"+ultimate+"_L"+angularMomentum+
            whetherForward+"_"+AlThickness+"mil"+(int)Math.round(Ebeam)+"MeV"
            +"_"+theta+"deg";
            thetaR = Math.toRadians(theta);
            System.out.println(target+"("+beam+","+projectile+") -> "+ultimate+"+"+
            decay);
            ...
            System.out.println("Ex = "+ExResid+" MeV, CM K.E. for detected decay product = "+
            (EcmDecay-Mdecay)+" MeV");
            MicronDetector md = new MicronDetector(z0,Math.toRadians(thetaYLSA));
            Solid deadLayer; EnergyLoss deadLayerLoss;
            try {
                outDescription.write(target+"("+beam+","+projectile+")"+residual+
                "("+decay+")"+ultimate+"\n");
                outDescription.write("Ex(residual "+residual+") = "+ExResid+
                " MeV\n");
                outDescription.write("Ex(final "+ultimate+") = "+ExUltimate+
                " MeV\n");
                outDescription.write("Theta(projectile "+projectile+") = "+theta+
                " degrees\n");
                ...
                /*** work begins here ***/
                Reaction reaction; Boost labBoost; Direction directionCM;
                int angleBinning=60;
                double divFactor = 180.0/angleBinning;
                int [] thetaCM = new int[angleBinning];
                int [] thetaLab = new int[angleBinning];
                int hits=0;
                outEvn.write("hit\tprojTheta\tprojPhi\tCMtheta\tCMphi\tlabTheta\tlabPhi\tinc.\tEdep\ttof\tdet\tstrip\n");
                int _i=0;
            ...
            do {
                ...
                double targetLoss = 0.001 *
                targetLossCalc.getThinEnergyLoss(beam,Ebeam);
                ...
                while (!directionAccepted){
                	...
                	//tan(theta)cos(phi)
                	double tempTanTheta = Math.abs(Math.tan(_randomProjDir.getTheta())*
                	Math.cos(_randomProjDir.getPhi()));
                	double tempTanPhi = Math.abs(Math.tan(_randomProjDir.getTheta())*
                ...
                reaction = new Reaction(target,beam,projectile,Tbeam,
                projDir.getThetaDegrees(),ExResid);
                ...
                //phi is opposite the phi of the projectile
         		Direction residDir = new Direction(
         		Math.toRadians(Math.abs(reaction.getLabAngleResidual(0))),
                ...
                if (!arrayForward) labBoost = Boost.inverseBoost(labBoost);
                ...
                int strip, det; double Edep,tof;
                if (arrayForward) {
                    directionLab=new Direction(Math.PI-directionLab.getTheta(),directionLab.getPhi());
                    directionCM = new Direction(Math.PI-directionCM.getTheta(),directionCM.getPhi());
                }
                ...
                if (hit) {
                    ...
                    double Tflight = Tinit - 0.001*
                    targetLossCalc.getThinEnergyLoss(decay,Tinit,
                    Math.PI-directionLab.getTheta());
                    tof = md.getDistance()/(Reaction.getBeta(decay,Tflight)*
                    C_MM_PER_NSEC);
                    Edep = Tflight - 0.001*deadLayerLoss.getEnergyLoss(decay,
                    Tflight, Math.acos(1/incidence));
                    boolean energyHit = Edep >= hit_threshold;
                    if (energyHit) {
                        hits++;
                        counts[bin]++;
                        if (Emin[bin] == 0.0 || Emin[bin] > Edep) Emin[bin]=Edep;
                        if (Emax[bin] == 0.0 || Emax[bin] < Edep) Emax[bin]=Edep;
                        //time-of-flight in nsec
                        outEvn.write("1\t"+round(projDir.getThetaDegrees())+"\t"+
                        round(projDir.getPhiDegrees())+"\t"+round(thetaDcm)+
                        "\t"+round(directionCM.getPhiDegrees())+"\t"+
                        round(thetaDlab)+"\t"+
                        round(directionLab.getPhiDegrees())+"\t"+round(incidence)+"\t"+
                        round(Edep)+"\t"+
                        round(tof)+"\t"+det+"\t"+strip+"\n");
                    } else {
                ...
                if (_i == numEvents && hits < hitsWanted){
                	...
                	System.out.println("Events so far = "+_i+", "+hits+" hits.");
                	System.out.println("Changing total events to simulate to "+numEvents+".");
                }
            } while (_i < numEvents);
                outEvn.flush(); outEvn.close();
                outCounts.write("bin\tdet\tstrip\tcounts\tEmin\tEmax\n");
                for (int i=0; i<5; i++){
                    for (int j=0; j<16; j++) {
                        int bin = i*16+j;
                        outCounts.write(bin+"\t"+i+"\t"+j+"\t"+counts[bin]+"\t"+
                        round(Emin[bin])+"\t"+round(Emax[bin])+"\n");
                    }
                }
                outAngles.write("Theta\tCMcounts\tlabCounts\n");
                for (int i=0; i<angleBinning; i++) {
                    outAngles.write((i*180/angleBinning)+"\t"+round(thetaCM[i])+"\t"+
                    round(thetaLab[i])+"\n");
                }
                outAngles.flush(); outAngles.close();
                outCounts.flush(); outCounts.close();
                System.out.println("Done. "+hits+" detector hits for "+_i+
                " simulated decays.");
                ...
                outDescription.flush();outDescription.close();
