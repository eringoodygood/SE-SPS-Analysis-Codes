...
//		Direction generator = new Direction(0, 0);
		...
		double[] ExResidValues = {6.343,  6.414,  6.496, 6.680, 6.695, 7.153, 7.399, 7.497, 7.648};//AL 5.940, 5.562 , 6.350, 6.38, 6.470, 6.789, 6.880, 7.150, 7.390, 7.498, 7.695};
...
//		int numEvents = (int) Math.round(hitsWanted * 1.2);//total events to simulate
		...
		target = new Nucleus(12, 26);
		beam = new Nucleus(2, 3);
		projectile = new Nucleus(1, 3);
		decay = new Nucleus(1, 1);
		Ebeam = 21; //beam kinetic energy in MeV
		xtarg = 40; //thickness in ug/cm^2
		String starg = "Mg 1"; //material specification for target
		...
		double AlThickness = 0;//not used
		double hit_threshold = 0.1;//energy deposited in MeV to be considered a hit
		...
		for (int rEx = 0; rEx < ExResidValues.length; rEx++) {
			...
			String outFileRoot =
				"C:/Documents and Settings/Owner/My Documents/Catherine/26Al(3He,t)26Si/YLSA simulations/"
...
"hit,projTheta,projPhi,CMtheta,CMphi,labTheta,labPhi,inc.,Edep,tof,det,strip\n");
				...
				do {
					...
					double targetLoss =
						0.001 * targetLossCalc.getThinEnergyLoss(beam, Ebeam);
					...
					if (hit) {
						...
						double Tflight =
							Tinit
								- 0.001
									* targetLossCalc.getThinEnergyLoss(
										decay,
										Tinit,
										Math.PI - directionLab.getTheta());
						tof =
...
/*					if (_i == numEvents && hits < hitsWanted) {
				...
				for (int i=0; i<stripCMtheta.length; i++){
					...
					outDescription.write(i+"\t"+round(stripCMtheta[i].getMean())+"\t"
					+_hits+"\t"+_eff+"\t"+_deleff+"\n");
				}
	...
	/**
	 * Returns thickness of Al in front of active region in cm (foil plus dead
layer).
